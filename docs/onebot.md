...
4. 创建 **QQ** 渠道并选择系统模板，绑定供应商账号和供应商模板，将供应商变量 `content` 映射到系统变量 `content`。QQ 支持可选签名，不配置签名也可以发送。
...
签名沿用短信的通道映射机制。`signature_name` 是通道中的稳定别名，实际发送的文字取自选中账号的签名资源 `signature_code`，不会直接显示别名。

1. 在 OneBot 账号的“签名”中创建本地资源：实际发送值填 `木雷科技`，后台名称可以填 `通知签名`。OneBot 签名无需供应商审核。
2. 在 QQ 渠道的“签名映射”中添加别名 `notice`，映射到这个资源。渠道绑定多个可选账号时，为每个账号配置相同别名。
3. 在渠道测试中选择“签名（可选）”，或在单发、批量 API 中传入 `signature_name`。
...
```json
{
  "channel_id": 42,
  "receiver": "group:987654",
  "signature_name": "notice",
  "template_params": { "content": "服务已恢复" }
}
```

最终消息为 `【木雷科技】服务已恢复`。如果沿用上文的供应商模板 `通知：{content}`，最终消息为 `【木雷科技】通知：服务已恢复`。签名首尾空白会去除，完整的 `【】` 不会重复包裹，签名和正文之间不额外加入空格或换行。

省略 `signature_name`、传空字符串或纯空白时，仍发送原正文，即使账号已经配置了签名，也不会自动添加。传入非空别名时，该别名必须在当前全部可选签名账号中具有有效映射；不存在、停用、删除或值为空时会拒绝发送。入队后映射失效，Worker 会终止该任务，不会改发无签名消息。

公共渠道详情仍返回 `signature_required: false`，但 `signature_names` 可以包含可选的共同别名；调用方应将选择框设置为可清空、默认不选。未配置共同别名不影响无签名发送。

在 CQ 模式下，签名中的 `&`、`[`、`]` 会按纯文本规则转义，正文 CQ 码正常解析；签名本身不会触发 @ 或图片等消息段。发送快照记录签名别名和映射值，请求日志记录带前缀的最终消息。
