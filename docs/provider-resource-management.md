...
占位符之外的 `$` 按原文保留。例如掌榕网模板 `主机${host_name}的规则{rule_name}` 识别的变量是 `host_name`、`rule_name`，变量标签为 `{host_name}`、`{rule_name}`；`$` 不属于变量名，也不会额外占用发送参数。报备转换后原文为 `主机${1}的规则{2}`。括号不完整、非法变量名和不支持的混合语法仍会报错。
...
新增或更新通道绑定时，启用且正常的绑定须通过上述可用性校验。校验失败返回 HTTP 400（响应 `code: 400`），`message` 说明具体原因；待审核模板提示“供应商模板尚未审核通过，请审核通过并同步后重试”。多个失败原因合并展示，数据库等内部故障仍返回 500。禁用或熔断的绑定保留原有参数映射编辑规则；审核通过并同步后可重试启用。此反馈不改变 readiness 原因码或实际发送资格。
