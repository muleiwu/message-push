...
- `signature_names` 是去重并排序后的公共别名，直接作为选择框的展示文字和提交值。它同时适用于短信签名和邮件标题。实际签名值、供应商账号及映射明细不在公开响应中。
- `signature_required` 与现有发送校验一致；为 `true` 时必须选择返回的别名。通道不可发送或没有必填签名的有效路径时，返回 `false` 和空选项。
- 多供应商通道只提供有效发送路径中需要签名的账号共同支持的别名，避免选择后因供应商切换而找不到映射。
