-- +goose Up
ALTER TABLE push_tasks ADD COLUMN provider_account_id BIGINT;
COMMENT ON COLUMN push_tasks.provider_account_id IS '最后一次发送尝试使用的服务商账号ID';
...
-- +goose Down
DROP INDEX IF EXISTS idx_push_tasks_provider_account;
