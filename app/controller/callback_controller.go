...
func (ctrl CallbackController) Handle(c httpInterfaces.RouterContextInterface) {
	idStr := c.Param("id")
	if idStr == "" {
		c.JSON(200, map[string]any{"code": 400, "message": "id is required"})
		return
	}
	...
	if err != nil {
		helper.GetRequestLogger(c).Error("invalid id: " + idStr)
		c.JSON(200, map[string]any{"code": 400, "message": "invalid id"})
		return
	}
	...
	if err != nil {
		helper.GetRequestLogger(c).Error("account not found: " + idStr)
		c.JSON(200, map[string]any{"code": 404, "message": "account not found"})
		return
	}
	...
	if err != nil {
		c.JSON(200, map[string]any{"code": 400, "message": "failed to read request body"})
		return
	}
