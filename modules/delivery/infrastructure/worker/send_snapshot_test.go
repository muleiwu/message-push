...
func TestHandleStoresExactPreparedContentForSuccessAndMissingResponse(t *testing.T) {
	...
	if _, err := registry.GetByCode(provider); err != nil {
		if err := registry.Register(&registry.ProviderMeta{Code: provider, Name: "快照测试", Type: "sms", SupportsSend: true}); err != nil {
			t.Fatal(err)
		}
	}
	for _, outcome := range []string{"success", "error_without_response", "nil_response"} {
		t.Run(outcome, func(t *testing.T) {
			...
			binding := &model.ChannelTemplateBinding{ID: 3, ParamMapping: `[{"type":"mapping","provider_var":"var1","system_var":"code"},{"type":"fixed","provider_var":"ttl","value":"5"}]`, ProviderTemplate: &model.ProviderTemplate{
				ID: 10, TemplateCode: "16021", TemplateName: "登录验证码", TemplateContent: "验证码{var1}，{ttl}分钟有效。", ContentType: "text",
			}}
			...
			h := &MessageHandler{
				...
				selector: snapshotSelector{node: &channel.ChannelNode{ChannelTemplateBinding: binding, ProviderAccount: &model.ProviderAccount{
