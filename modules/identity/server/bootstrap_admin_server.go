...
func (receiver *BootstrapAdminServer) Run() error {
	...
	if err := db.Model(&model.AdminUser{}).Count(&count).Error; err != nil {
		return fmt.Errorf("[bootstrap admin] 统计管理员失败: %w", err)
	}
	if count > 0 {
		// 已存在管理员，幂等跳过
		return nil
	...
	if err != nil {
		return fmt.Errorf("[bootstrap admin] 密码加密失败: %w", err)
	}
	...
	if err := db.Create(adminUser).Error; err != nil {
		return fmt.Errorf("[bootstrap admin] 创建初始管理员失败: %w", err)
	}

	logger.Info(fmt.Sprintf("[bootstrap admin] 已创建初始管理员: %s", username))
	return nil
}
